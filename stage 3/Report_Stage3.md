...
## 2.5 אלגוריתם הינדוס לאחור (Reverse Engineering)
כדי לייצר את תרשים ה-ERD מתוך הסכמה (ה-DSD) שקיבלנו, ביצענו את השלבים הבאים:
1. **זיהוי ישויות (Entities):** כל טבלה במסד הנתונים שקיבלנו מתורגמת לישות (למשל `BankClient`, `MortgageLoan`, `Appraisal`).
2. **זיהוי תכונות (Attributes):** כל עמודה בטבלה מתורגמת לתכונה של הישות. השדה שמוגדר כ-Primary Key (PK) הופך למזהה הייחודי של הישות.
3. **זיהוי קשרים (Relationships):** עברנו על אילוצי ה-Foreign Key (FK) בטבלאות. 
   - מפתח זר מצביע על קשר בין ישויות. לדוגמה, המפתח הזר בטבלת `MortgageLoan` שמצביע ל-`BankClient` מלמד על קשר של "לקוח לוקח משכנתא".
   - ריבוי הקשר (Cardinality) נקבע לפי אופי המפתח הזר. בדרך כלל, מפתח זר מייצג קשר של יחיד לרבים (1:N), כאשר הטבלה המכילה את ה-FK היא צד ה"רבים" והטבלה אליה הוא מפנה היא צד ה"יחיד".
4. **שרטוט התרשים:** חיבור הישויות, הוספת התכונות ומתיחת קווי הקשרים ביניהן לקבלת התרשים השלם.

## 2.6 בדיקת השאילתות משלב ב'
לאחר סיום שלב האינטגרציה ויצירת בסיס הנתונים המשולב הכולל את הנתונים החדשים והקיימים, הרצנו מחדש את כל השאילתות שנכתבו במסגרת שלב ב' של הפרויקט. 
וידאנו כי כל השאילתות רצות בצורה תקינה ללא שגיאות, ומחזירות את התוצאות המצופות על גבי הסכמה המאוחדת.
